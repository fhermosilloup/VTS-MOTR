...
from datasets import ua_detrac_coco
from datasets import ua_detrac_mot
...
from util.motdet_eval import motdet_evaluate
...
def get_args_parser():
    ...
    #Added for validation in MOT datasets
    #----------------------------------------------------
    parser.add_argument('--val_every', type=int, default=5,
                    help='Run MOT validation every N epochs (MOT datasets only)')
    parser.add_argument('--val_iou_thres', type=float, default=0.5,
                    help='IoU threshold for MOT validation (motdet_evaluate)')
    #----------------------------------------------------
...
def main(args):
    ...
    datasets2collate_fn = {
        ...
        'ua_detrac_coco' : utils.ua_detrac_collate_fn,
        'detrac_mot' : utils.mot_collate_fn
    }
    print("Collate for: ", args.dataset_file)
    ...
    best_mAP = -1.0  # track best validation mAP for MOT datasets
    
    dataset_list = ['e2e_mot', 'bdd100k_mot', 'detrac_mot']
    ...
    with EventStorage(args.start_epoch * len(dataset_train)) as storage:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # ===== MOT validation every N epochs =====
            if args.dataset_file in dataset_list and (epoch + 1) % args.val_every == 0 and utils.is_main_process():
                mAP, mR, mP = motdet_evaluate(model, data_loader_val,
                                              iou_thres=args.val_iou_thres, print_interval=50)
                if writer is not None:
                    writer.add_scalar("val/mAP", float(mAP), epoch + 1)
                    writer.add_scalar("val/Recall", float(mR), epoch + 1)
                    writer.add_scalar("val/Precision", float(mP), epoch + 1)

                if float(mAP) > best_mAP:
                    best_mAP = float(mAP)
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
                    ...
                    print(f"[MOT EVAL] New best mAP={best_mAP:.4f} @ epoch {epoch+1}. Saved best_mAP.pth")
                    
            evaluation_datasets = ['coco', 'ua_detrac_coco', 'detrac_mot']
            if args.dataset_file in evaluation_datasets:
