...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        data_dict = {'samples': samples, 'gt_instances': targets}
...
def train_one_epoch_mot(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_dict in metric_logger.log_every(data_loader, print_freq, header):
        ...
        #print("[DEBUG] - DataDict.len(): ", len(data_dict['imgs']))
        #print("[DEBUG] - DataDict[0].type(): ", type(data_dict['imgs'][0]))
        outputs = model(data_dict)
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        track_instances = outputs['track_instances'][0] if isinstance(outputs['track_instances'], list) else outputs['track_instances']
        ...
        loss_dict = criterion(outputs_for_criterion, targets)
        weight_dict = criterion.weight_dict
        ...
        if 'bbox' in postprocessors:
            ...
            # outputs['track_instances'] puede ser una única instancia o una lista
            track_list = outputs['track_instances']
            if not isinstance(track_list, (list, tuple)):
                track_list = [track_list]

            results = []
            for inst, size in zip(track_list, orig_target_sizes):
                # model.post_process espera tamaño en orden (H,W)
                processed = model.post_process(inst, size.tolist())
                # Adaptar al formato esperado por CocoEvaluator: dict con tensors
                results.append({
