...
class FrameMatcherV2(SetCriterion):
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        if len(active_track_boxes) > 0:
            ...
            tmp_box_iou = pairwise_iou(Boxes(active_track_boxes), Boxes(gt_boxes))
            iou_max, gt_match = tmp_box_iou.max(dim=1)
            track_instances.iou[active_idxes] = iou_max
...
def build(args):
    ...
    criterion.to(device)
    postprocessors = {}
    print("[DEBUG]: NumQueries:",args.num_queries)
    model = E2EMotDETRV3(
        backbone,
        transformer,
