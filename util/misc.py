...
# (Asegúrate de que la transformación esté definida)
transform = T.Compose([
    T.ToTensor(),
...
class _NewEmptyTensorOp(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        return _NewEmptyTensorOp.apply(grad, ctx.shape), None
...
# Esto reemplaza la mot_collate_fn cuando uses UA-DETRAC
...
def ua_detrac_collate_fn(batch):
    images = []
    targets = []
    for item in batch:
        # cargar imagen y aplicar transform
        img, ann = item
        ...
        if ann:
            ...
            obj_ids = ann["obj_ids"].clone().detach()
            iscrowd = ann["iscrowd"].clone().detach()
        else:
        ...
        # 3) Tamaños e identificadores requeridos por evaluate()
        size = ann["size"]
        image_id = int(ann["img_id"][0])
        ...
        targets.append({
            ...
            "size": size,        # tamaño actual (post-aug) — aquí igual al original
            "orig_size": size    # tamaño original
        })
    try:
        samples = nested_tensor_from_tensor_list(images)
    except AttributeError:
        sys.exit("Fatal error: something went wrong")
