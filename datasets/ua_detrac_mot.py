"""
DETRAC-MOT dataset dataloader (adapted for relative paths like MVI_20011/img/000001.jpg)
Compatible with BDD100K-style DetMOTDetection.
"""
...
class DetracMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, transforms):
        ...
        for img_path in self.img_files:
            # carpeta del video
            seq_folder = os.path.basename(os.path.dirname(os.path.dirname(img_path)))  # MVI_20012
            gt_path = os.path.join(seqs_folder, "train" if "train" in data_txt_path else "val", seq_folder, "gt", "gt.txt")
    ...
    def _register_videos(self):
        for label_path in self.label_files:
            video_folder = osp.dirname(label_path)
            if video_folder not in self.video_dict:
                video_idx = len(self.video_dict)
                print(f"register {video_idx + 1}-th video: {video_folder}")
                self.video_dict[video_folder] = video_idx
    ...
    def _pre_single_frame(self, idx: int):
        ...
        if osp.isfile(label_path):
            try:
                labels0 = np.loadtxt(label_path, dtype=np.float32, delimiter=',', ndmin=2).reshape(-1, 9)
            except ValueError:
                print(f"Error in file {label_path} at frame {idx+1}")
                sys.exit(-1)

            # normalized cewh to pixel xyxy format
            # frame,id,x,y,w,h,conf,class,vis
            labels = labels0[labels0[:, 0] == (frame_idx),:].copy()
            labels[:, 4] += labels[:, 2]
            labels[:, 5] += labels[:, 3]
        else:
            raise ValueError('invalid label path: {}'.format(label_path))
        video_name = osp.dirname(label_path)
        
        obj_idx_offset = self.video_dict[video_name] * 100000  # 100,000 unique ids is enough for a video.
        targets['boxes'] = []
        targets['area'] = []
        targets['iscrowd'] = []
        targets['labels'] = []
        ...
        #print(f"category_ids from {label_path} and frame {frame_idx}: {np.unique(labels[:,7])}")
        for label in labels:
            targets['boxes'].append(label[2:6].tolist())
            targets['area'].append(label[4] * label[5])
            targets['iscrowd'].append(int(0))
            targets['labels'].append(int(label[7])-1) # category start from 0
            obj_id = label[1] + obj_idx_offset if label[1] >= 0 else label[1]
            targets['obj_ids'].append(int(obj_id))  # relative id
...
def make_detracmot_transforms(image_set, args=None):
...
def build_detrac_mot(image_set, args):
    print("[DEBUG]: UA-DETRAC mot version selected")
    root = Path(args.mot_path)
    assert root.exists(), f"provided DETRAC-MOT path {root} does not exist"
    transforms = make_detracmot_transforms(image_set, args)

    if image_set == 'train':
        data_txt_path = args.data_txt_path_train
        print("[DEBUG]: UA-DETRAC-MOT Train dataset selected")
    elif image_set == 'val':
        data_txt_path = args.data_txt_path_val
        print("[DEBUG]: UA-DETRAC-MOT Validation dataset selected")
    else:
        raise ExceptionType("Invalid UA-DETRAC dataset")
    
    return DetracMOTDetection(args, data_txt_path=data_txt_path, seqs_folder=root, transforms=transforms)
